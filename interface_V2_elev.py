...
class App(ctk.CTk):
    ...
    def _build_graph(self, params):
        try:
            n_maisons = int(params["n_maisons"])
        except:
            n_maisons = 10
        try:
            n_chateaux = max(1, int(params["n_chateaux"]))
        except:
            n_chateaux = 1
        try:
            zone = params["Zone"]
        ...
        if zone == "Rurale":
            order = math.trunc(math.log(n_maisons) / math.log(
                2))  # rajouter des maisons au hasard (difference entre puissance de 2 et nb voulu)
            ...
            pos = nx.spring_layout(G, k=0.3, iterations=300,
                                   seed=30)  # permet d'avoir une forme où les noeuds se croisent le moins possible
            for node in G.nodes():
            ...
            if n_maisons != 2 ** order:
                ...
                for i in range(0, ajout):
                    ...
                    pos[f"J{nouv_noeud}"] = np.array([pos[noeud_ajout][0] + 0.2 * random.uniform(-1, 1),
                                                      pos[noeud_ajout][1] + 0.2 * random.uniform(-1, 1)])
            ...
            for i in range(1, n_chateaux + 1):
                ...
                pos[chateau_node] = np.array([pos[noeud_chateau][0] + 0.2 * random.uniform(-1, 1),
                                              pos[noeud_chateau][1] + 0.2 * random.uniform(-1, 1)])
            pos = nx.spring_layout(G, k=0.3, iterations=500,
                                   seed=30)  # permet d'avoir une forme où les noeuds se croisent le moins possible, avant d'ajouter les maisons

            # Pour rajouter une maison (un noeud) à chaque noeud deja existant, permet d'ajouter la maison à angle droit avec le tuyau
            for i in list(G.nodes())[0:n_maisons]:
                parent = next(G.neighbors(i))
                edge_vector = np.array(pos[i]) - np.array(pos[parent])
                perp_vector = np.array([-edge_vector[1], edge_vector[0]]) * random.choice([-1,
                                                                                           1])  # on multiplie par 1 ou -1 pour avoir les maisons aléatoirement à droite ou à gauche de la rue
                perp_vector = perp_vector / np.linalg.norm(
                    perp_vector) * 0.05  # à modifier si envie, c'est la distance du tuyau jusqu'à la maison
            ...
            pos = nx.spring_layout(G, k=0.3, iterations=iter,
                                   seed=30)  # permet d'avoir une forme où les noeuds se croisent le moins possible, avant d'ajouter les maisons
    ...
    def on_ok(self):
        ...
        # Vérifier hauteurs
        try:
            hauteur_min = int(params["Hauteur min"])
            hauteur_max = int(params["Hauteur max"])
    ...
    def on_random(self):
        ...
        for key, val in {
            ...
            "Nombre de chateaux d'eau": str(rng.randint(1, 10)),
            ...
            "Hauteur min": str(rng.choice([0, 80, 100])),
            "Hauteur max": str(rng.choice([100, 200, 300])),
        }.items():
            e = self.entries[key]
    ...
    def _build_inp(self):
        ...
        try:
            ...
            surface = 2.0
        try:
            hauteur_min = float(p.get("Hauteur min", 100))
        except:
            hauteur_min = 100.0
        try:
            hauteur_max = float(p.get("Hauteur max", 150))
        except:
            hauteur_max = 150.0
        ...
        for i, (node, _) in enumerate(all_juncs):
            ...
            if pos_xy is not None:
                x, y = pos_xy
                elev = round(get_elevation(x / 100.0, y / 100.0, hauteur_min, hauteur_max), 2)
            else:
        ...
        for node, _ in chateaux:
            ...
            if pos_xy is not None:
                x, y = pos_xy
                hauteur = 15 + round(get_elevation(x / 100.0, y / 100.0, hauteur_min, hauteur_max), 2)
            else:
